...
\[
  f(x) = \exp(x) - 1,
\]
on a computer (in double precision arithmetic) for very small values of the
argument, i.e., for \(0 < \abs{x} \ll 1\).

The exercise asks us to:
\begin{enumerate}
  \item Perform a numerical experiment in double precision using values
        \(x = 10^{-k}\) for \(k = 1,2,3,\dots\). We may assume that
        \(\exp(x)\) itself is implemented correctly in double precision.
        As an ``almost exact'' reference value for \(f(x)\), we are asked to
        use the Taylor expansion of \(\exp(x)\) about \(x = 0\), for example
        truncated after the 10th degree. We shall then plot the relative
        error between the direct evaluation \(\exp(x)-1\) and the Taylor
        reference on a logarithmic scale.
  \item Explain why the direct evaluation of \(f(x)\) in the form
        \(\exp(x)-1\) is numerically unstable for \(\abs{x} \to 0\).
\end{enumerate}
...
\]
In our computations we use \(N = 10\) as a brute-force reference for small
\(\abs{x}\).

The relative error we report is
\[
  \mathrm{rel.\ error}(x)
  = \frac{\abs{f_{\mathrm{direct}}(x) - f_{\mathrm{ref}}(x)}}
         {\abs{f_{\mathrm{ref}}(x)}},
\]
where \(f_{\mathrm{direct}}(x) = \exp(x) - 1\) uses the built-in exponential
function in double precision, and \(f_{\mathrm{ref}}(x)\) is the Taylor
reference.
...
\]
In exact arithmetic, subtracting 1 would give
\(\exp(x)-1 = x + \mathcal{O}(x^2)\), so the result is of order \(\abs{x}\).

In floating point arithmetic, however, we can model the situation as follows.
For small \(\abs{x}\), the correctly rounded value of \(\exp(x)\) can be written as
\[
  \mathrm{fl}(\exp(x)) = 1 + x + \delta,
\]
where \(\delta\) is a round-off error of order \(\mathcal{O}(\varepsilon)\), with
\(\varepsilon\) the machine precision. When we form
\[
  \mathrm{fl}(\exp(x)-1)
  = \mathrm{fl}\bigl( (1 + x + \delta) - 1 \bigr)
  \approx x + \delta,
\]
the large leading terms \(1\) cancel, and we are left with the much smaller
difference \(x + \delta\). The absolute error is still on the order of
\(\abs{\delta} \approx \varepsilon\), but the exact value \(\exp(x)-1 \approx x\) has
magnitude \(\mathcal{O}(\abs{x})\). Hence the relative error behaves like
\[
  \frac{\abs{(x + \delta) - x}}{\abs{x}}
  = \frac{\abs{\delta}}{\abs{x}}
  \sim \frac{\varepsilon}{\abs{x}},
\]
which grows rapidly as \(\abs{x} \to 0\). For sufficiently small \(x\),
\(\exp(x)\) may even round to exactly \(1\), so the computed value of
\(\exp(x)-1\) is zero, while the exact value is nonzero.

This is an example of \emph{catastrophic cancellation}: subtracting nearly
equal numbers eliminates most significant digits from the result and amplifies
the effect of rounding errors, even though the underlying mathematical
problem (computing \(\exp(x)-1\)) is well-conditioned.

\section{Conclusion}
The direct evaluation of \(\exp(x)-1\) in double precision is numerically
unstable for small \(\abs{x}\) because of cancellation of leading digits.
Although \(\exp(x)\) itself is computed accurately, the subtraction of 1 causes
the relative error in the final result to grow like \(\varepsilon / \abs{x}\) as
\(\abs{x} \to 0\).

A numerically stable alternative near zero is to use a series expansion such
as the truncated Taylor series, or to call a dedicated routine like
\verb|expm1(x)| which is implemented to avoid cancellation in this regime.
